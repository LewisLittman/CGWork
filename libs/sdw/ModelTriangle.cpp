...
ModelTriangle::ModelTriangle(const glm::vec3 &v0, const glm::vec3 &v1, const glm::vec3 &v2, Colour trigColour) :
		vertices({{v0, v1, v2}}), texturePoints(), colour(std::move(trigColour)), normal(), vertexNormals(), lighting(), mirror(false), shadows(false), normalMap(false), texture(false), shadingMode(0) {}
